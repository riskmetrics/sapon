...
public class OperationContext extends AbstractContext<ServiceContext>
     ...
     */
    private transient Map<String, MessageContext> messageContexts;
     ...
     */
    private Map<String, MetaDataEntry> metaMessageContextMap = null;

    /**
     * This is used to hold temporarily any message context objects
     * that were isolated from the messageContexts map.
     */
    private transient Map<String, MessageContext> isolatedMessageContexts = null;

    /**
     * This is used to hold temporarily any message context objects
     * from the messageContexts map for save/restore activities.
     */
    private transient Map<String, MessageContext> workingSet = null;
    ...
    public ConfigurationContext getConfigurationContext() {
        if (parent != null) {
            return (parent).getConfigurationContext();
        } else {
            return null;
        }
    }
    ...
    */
    ...
    public void writeExternal(ObjectOutput o) throws IOException {
        ...
        Map<String, MessageContext> tmpMsgCtxMap = null;

        if ((messageContexts != null) && (!messageContexts.isEmpty())) {
            // create a table of the non-isolated message contexts
            workingSet = new HashMap<String, MessageContext>();
            tmpMsgCtxMap = new HashMap<String, MessageContext>();

            //Set<String> keySet = messageContexts.keySet();

            for(final Map.Entry<String, MessageContext> e: messageContexts.entrySet()) {
                // get the message context associated with that label
            	final String key = e.getKey();
                final MessageContext value = e.getValue();
                ...
                if (isolatedMessageContexts != null) {
                    if (!isolatedMessageContexts.isEmpty()) {
                        ...
                        final MessageContext valueIsolated =
                                isolatedMessageContexts.get(key);
            ...
            for(final Map.Entry<String, MessageContext> e: workingSet.entrySet()) {
                final String key = e.getKey();
                final MessageContext mc = e.getValue();
                ...
                if (log.isTraceEnabled()) {
                    log.trace(getLogCorrelationIDString() +
                              ":writeExternal():  getting working set entry  key [" + key +
    ...
    public void readExternal(ObjectInput inObject) throws IOException, ClassNotFoundException {
        ...
        // trace point
        log.trace(getClass().getName() + ":readExternal():  BEGIN  bytes available in stream [" +
                in.available() + "]  ");
        ...
        // trace point
        if (log.isTraceEnabled()) {
            log.trace(getClass().getName() + ":readExternal():  reading input stream for [" +
        ...
        properties = in.readMap(new HashMapUpdateLockable<String, Object>());
        ...
        messageContexts = new HashMap<String, MessageContext>();
        in.readUTF(); // read marker
    ...
    public void activate(ConfigurationContext cc) {
        ...
        if (metaParent != null) {
            ...
            if (existingSC == null) {
                ...
                if (existingOC != null) {
                    existingSC = existingOC.getParent();
                }
            }
        ...
        if (key != null) {
            ...
            if (isOutbound(mepString) && !isComplete) {
	...
	private boolean isOutbound(String mepString) {
    	return mepString.equals(WSDL20_2006Constants.MEP_URI_OUT_ONLY)
    ...
    public void isolateMessageContext(MessageContext mc) {
        ...
        for(final Map.Entry<String, MessageContext> e: messageContexts.entrySet()) {
            ...
            MessageContext value = e.getValue();
            ...
            if (value != null) {
                ...
                if ((valueID != null) && valueID.equals(messageID)) {
                    ...
                    if (metaMessageContextMap == null) {
                        metaMessageContextMap
                        	= new HashMap<String, MetaDataEntry>();
                    }
                    ...
                    if (isolatedMessageContexts == null) {
                        isolatedMessageContexts
                        	= new HashMap<String, MessageContext>();
                    }
    ...
    public void restoreMessageContext(MessageContext msg) {
        ...
        if (metaMessageContextMap != null) {
            for(final Map.Entry<String, MetaDataEntry> e: metaMessageContextMap.entrySet()) {
                final String keyM = e.getKey();
                final MetaDataEntry valueM = e.getValue();
                String valueM_ID;
        ...
        else if (messageContexts != null) {
        	for(final Map.Entry<String, MessageContext> e: messageContexts.entrySet()) {
        		final String key = e.getKey();
        		final MessageContext value = e.getValue();
        		if (value != null) {
        			if (msgID.equals(value.getMessageID())) {
    ...
    public String getLogCorrelationIDString() {
        if (logCorrelationIDString == null) {
            logCorrelationIDString = getClass().getName() + "@" + UUIDGenerator.getUUID();
        }
