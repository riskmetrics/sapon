...
public class Aggregate extends TimerTask {
    ...
    private List<SynapseMessageContext> messages = new ArrayList<SynapseMessageContext>();
     ...
     */
    public synchronized boolean addMessage(SynapseMessageContext synCtx) {
    ...
    public synchronized boolean isComplete() {
        ...
        if (!completed) {
            ...
            if (!messages.isEmpty()) {
                ...
                SynapseMessageContext mc = messages.get(0);
                ...
                log.debug("No messages collected in current aggregation");
            }
            ...
            if (expiryTimeMillis > 0 && System.currentTimeMillis() >= expiryTimeMillis) {
                log.debug("Aggregation complete - the aggregation has timed out");
    ...
    public List<SynapseMessageContext> getMessages() {
        return messages;
    }

    public void setMessages(List<SynapseMessageContext> messages) {
