...
public class ModuleBuilder extends DescriptionBuilder {
            ...
            if ((moduleClassName != null) && !"".equals(moduleClassName)) {
                ...
                try {
                    AccessController.doPrivileged(new PrivilegedExceptionAction<Class<?>>() {
                        public Class<?> run() throws IllegalAccessException, InstantiationException {
    ...
    public void populateModule() throws DeploymentException {
        try {
            ...
            Iterable<OMElement> itr = moduleElement.getChildrenWithName(new QName(TAG_PARAMETER));
            ...
            Iterable<OMElement> phaseItr = moduleElement.getChildrenWithName(new QName(TAG_PHASE));
            ...
            */
            ...
            Iterable<OMElement> op_itr = moduleElement.getChildrenWithName(new QName(TAG_OPERATION));
...
//          processing <wsp:Policy> .. </..> elements
     ...
     */
    private void processModulePhase(Iterable<OMElement> phases) throws AxisFault {
        if (phases == null){
            return;
        }

        for(OMElement element: phases) {
