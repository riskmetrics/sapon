...
public class WSDLReaderImpl implements WSDLReader
{
  ...
  private static final List<String> STYLE_ONE_WAY =
    Arrays.asList(new String[]{Constants.ELEM_INPUT});
  private static final List<String> STYLE_REQUEST_RESPONSE =
    Arrays.asList(new String[]{Constants.ELEM_INPUT, Constants.ELEM_OUTPUT});
  private static final List<String> STYLE_SOLICIT_RESPONSE =
    Arrays.asList(new String[]{Constants.ELEM_OUTPUT, Constants.ELEM_INPUT});
  private static final List<String> STYLE_NOTIFICATION =
  ...
  protected Map<String, Schema> allSchemas = new Hashtable<String, Schema>();
  ...
  protected Definition parseDefinitions(String documentBaseURI,
                                        Element defEl,
                                        Map<String, Definition> importedDefs)
                   ...
                   throws WSDLException
  {
     ...
     */
      ...
      //into a single list
  	  ...
  	  List<SchemaReference> allSchemaRefs = new ArrayList<SchemaReference>();

  	  for(List<SchemaImport> imports: schema.getImports().values()) {
  		allSchemaRefs.addAll(imports);
	  }
  	  ...
  	  for(SchemaReference schemaRef: allSchemaRefs) {
        try {
  	      ...
  	      if (loc != null)
  	      {
  	        ...
  	        inputSource = loc.getImportInputSource(
  	  	    ...
  	  	    referencedSchema = this.allSchemas.get(location);
  	      }
  	      else
   	      {
    ...
    throws WSDLException
  {
    ...
    List<Attr> remainingAttrs = DOMUtils.getAttributes(bindingEl);
      ...
      throws WSDLException
  {
    ...
    List<Attr> remainingAttrs = DOMUtils.getAttributes(bindingOperationEl);
                                             ...
                                             throws WSDLException
  {
    ...
    List<Attr> remainingAttrs = DOMUtils.getAttributes(bindingInputEl);
                                               ...
                                               throws WSDLException
  {
    ...
    List<Attr> remainingAttrs = DOMUtils.getAttributes(bindingOutputEl);
                                             ...
                                             throws WSDLException
  {
    ...
    List<Attr> remainingAttrs = DOMUtils.getAttributes(bindingFaultEl);
    ...
    throws WSDLException
  {
    ...
    List<Attr> remainingAttrs = DOMUtils.getAttributes(msgEl);
    ...
    for (int i = 0; i < length; i++)
    {
      ...
      if (namespaceURI != null && !namespaceURI.equals(Constants.NS_URI_WSDL))
      {
        if (!namespaceURI.equals(Constants.NS_URI_XMLNS))
        {
          ...
          List<QName> val = parseExtensibilityAttribute(el, strValue, def);
  ...
  protected List<QName> parseExtensibilityAttribute(Element el,
                                               String attrValue,
                                               Definition def)
                                                 throws WSDLException
  {
      ...
      List<QName> newList = new Vector<QName>(size);
                                       ...
                                       throws WSDLException
  {
    ...
    List<Attr> remainingAttrs = DOMUtils.getAttributes(opEl);
    ...
    List<String> messageOrder = new Vector<String>();
    Element docEl = null;
    Input input = null;
    Output output = null;
    List<Fault> faults = new Vector<Fault>();
    List<ExtensibilityElement> extElements = new Vector<ExtensibilityElement>();
    ...
    if (extElements.size() > 0) {
      for(ExtensibilityElement extElem: extElements) {
    ...
    throws WSDLException
  {
    ...
    List<Attr> remainingAttrs = DOMUtils.getAttributes(serviceEl);
    ...
    throws WSDLException
  {
    ...
    List<Attr> remainingAttrs = DOMUtils.getAttributes(portEl);
