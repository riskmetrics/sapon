...
public class MessageContextBuilder {
    ...
    public static MessageContext createFaultMessageContext(MessageContext processingContext,
        ...
        List<OMElement> soapHeadersList = getSoapHeaderElems(processingContext);

        if (soapHeadersList != null) {
            SOAPHeader soapHeaderElement = envelope.getHeader();
            for (OMElement soapHeaderBlock : soapHeadersList) {
	...
	private static List<OMElement> getSoapHeaderElems(MessageContext processingContext) {
    	return (List<OMElement>) processingContext.getProperty(SOAPConstants.HEADER_LOCAL_NAME);
    }
