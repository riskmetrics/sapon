...
/**
 * All the engine components are stateless across the executions and all the
 * states should be kept in the Contexts, there are three context Global,
 * Session and Message.
 */
public class SessionContext<P extends Context<?>> extends AbstractContext<P>
    implements Externalizable, SafeSerializable
{
     ...
     * @param parent
     */
    public SessionContext(P parent) {
        super(parent);
    }
    ...
    public ServiceContext getServiceContext(AxisService axisService) {
        return serviceContextMap.get(axisService.getName());
    }
    ...
    public ServiceGroupContext getServiceGroupContext(String serviceGroupID) {
        return serviceGroupContextMap.get(serviceGroupID);
    }
    ...
    public Iterable<ServiceGroupContext> getServiceGroupContext() {
        if (serviceGroupContextMap != null) {
            if (serviceGroupContextMap.isEmpty()) {
                return null;
            }
            return serviceGroupContextMap.values();
        } else {
            return null;
        }
    }
	...
	protected void finalize() throws Throwable {
        ...
        if (serviceGroupContextMap != null && !serviceGroupContextMap.isEmpty()) {
            ...
            while (valuse.hasNext()) {
                ServiceGroupContext serviceGroupContext = valuse.next();
    ...
    private void cleanupServiceContextes(ServiceGroupContext serviceGroupContext) {
        for(final ServiceContext serviceContext:
    ...
    */
	...
	private P readParent(ObjectInput in)
