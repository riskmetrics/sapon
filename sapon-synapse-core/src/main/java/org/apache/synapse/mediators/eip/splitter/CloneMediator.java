	...
	implements ManagedLifecycle
{
    ...
    public boolean mediate(SynapseMessageContext synCtx) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("Start : Clone mediator");
        ...
        while (iter.hasNext()) {
            if (log.isDebugEnabled()) {
                log.debug("Submitting " + (i+1) + " of " + targets.size() +
     ...
     */
    private SynapseMessageContext getClonedMessageContext(SynapseMessageContext synCtx, int messageSequence,
        int messageCount) {
