...
public class AddressingInHandler extends AbstractHandler implements AddressingConstants {
    ...
    public InvocationResponse invoke(MessageContext msgContext) throws AxisFault {
        ...
        if (namespace == null) {
            ...
            iterator = header.getHeadersToProcess(rolePlayer, namespace).iterator();
     ...
     */
    private void extractAddressingInformation(	MessageContext messageContext,
    											Iterator<SOAPHeaderBlock> headers,
    											String namespace)
            throws AxisFault {
        Options messageContextOptions = messageContext.getOptions();

        // Normally will not be used for more than 1 header
        List<String> duplicateHeaderNames = new ArrayList<String>(1);

        List<SOAPHeaderBlock> relatesToHeaders = null;
        ...
        while (headers.hasNext()) {
            SOAPHeaderBlock soapHeaderBlock = headers.next();
        ...
        if (relatesToHeaders != null) {
            for (int i = 0; i < relatesToHeaders.size(); i++) {
                extractRelatesToInformation(relatesToHeaders.get(i),
                                            messageContextOptions);
            }
        }
    ...
    private void checkDuplicateHeaders(String addressingHeaderName, int headerFlag,
                                       ...
                                       List<String> duplicateHeaderNames) {//throws AxisFault {
    ...
    private void extractRelatesToInformation(SOAPHeaderBlock soapHeaderBlock,
        ...
        List<OMAttribute> attributes = extractAttributesFromSOAPHeaderBlock(soapHeaderBlock);
    ...
    private void extractToEPRInformation(SOAPHeaderBlock soapHeaderBlock,
        ...
        Iterator<OMAttribute> addressAttributes
        	= soapHeaderBlock.getAllAttributes().iterator();
        if (addressAttributes != null && addressAttributes.hasNext()) {
            ...
            while (addressAttributes.hasNext()) {
                OMAttribute attr = addressAttributes.next();
                attributes.add(attr);
            }
    ...
    private void extractToEprReferenceParameters(EndpointReference toEPR, SOAPHeader header,
                                                   String namespace) {
        if (Final.WSA_NAMESPACE.equals(namespace)) {
            for(OMElement headerElement: header.getChildElements()) {
    ...
    private List<OMAttribute> extractAttributesFromSOAPHeaderBlock(SOAPHeaderBlock soapHeaderBlock) {
        Iterator<OMAttribute> actionAttributes = soapHeaderBlock.getAllAttributes().iterator();
        if (actionAttributes != null && actionAttributes.hasNext()) {
            List<OMAttribute> attributes = new ArrayList<OMAttribute>();
            while (actionAttributes.hasNext()) {
                OMAttribute attr = actionAttributes.next();
                attributes.add(attr);
            }
            return attributes;
        }
