...
public class ProxyServiceSerializer {
    ...
    protected static final OMNamespace synNS = fac.createOMNamespace(XMLConfigConstants.SYNAPSE_NAMESPACE, "");
    ...
    public static OMElement serializeProxy(OMElement parent, ProxyService service) {
        ...
        List<String> transports = service.getTransports();
        if (transports != null && !transports.isEmpty()) {
            StringBuilder transportStr = new StringBuilder(transports.get(0));
            for (int i = 1; i < transports.size(); i++) {
                transportStr.append(" " + transports.get(i));
            }
            proxy.addAttribute(fac.createOMAttribute("transports", nullNS, transportStr.toString()));
        }
        ...
        for (PolicyInfo pi : service.getPolicies()) {
            ...
            if (pi instanceof ServicePolicyInfo) {
            	policy.addAttribute(fac.createOMAttribute("where", nullNS, "service"));
            }

            if (pi instanceof OperationPolicyInfo) {
            	OperationPolicyInfo opi = (OperationPolicyInfo)pi;
                policy.addAttribute(fac.createOMAttribute("operationName", nullNS, opi.getOperation().getLocalPart()));
                if (opi.getOperation().getNamespaceURI() != null) {
                    policy.addAttribute(fac.createOMAttribute(
                            "operationNamespace", nullNS, opi.getOperation().getNamespaceURI()));
                }
            }

            if (pi instanceof MessagePolicyInfo) {
            	MessagePolicyInfo mpi = (MessagePolicyInfo)pi;
                policy.addAttribute(fac.createOMAttribute("type", nullNS, mpi.getMessageLabel().toLowerCase()));
            }
