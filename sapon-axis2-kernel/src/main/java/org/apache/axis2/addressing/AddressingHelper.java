...
public class AddressingHelper {
    ...
    public static boolean shouldSendFaultToFaultTo(MessageContext messageContext) {
        ...
        if (faultInfoForHeaders != null) {
            ...
            String problemHeaderName = ((Map<String, String>)faultInfoForHeaders)
    ...
    /**
     * Set the value of an existing unlocked Parameter representing Anonymous
     * or add a new one if one does not exist. If a locked Parameter of the
     * same name already exists the method will trace and return.
     *
     * @param axisOperation
     * @param value
     */
    public static void setInvocationPatternParameterValue(AxisOperation axisOperation, String value) {
        if (value == null) {
            if (LoggingControl.debugLoggingAllowed && log.isDebugEnabled()) {
                log.debug("Value passed in is null. return");
            }
            return;
        }
        ...
        if (param != null) {
            if (LoggingControl.debugLoggingAllowed && log.isDebugEnabled()) {
                log.debug("Parameter " + param.getName() + " already exists," +
