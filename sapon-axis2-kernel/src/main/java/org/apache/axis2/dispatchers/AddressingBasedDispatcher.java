	...
	implements AddressingConstants
{
    ...
    private final RequestURIBasedServiceDispatcher serviceDispatcher
    	= new RequestURIBasedServiceDispatcher();
    private final ActionBasedOperationDispatcher operationDispatcher
	...
	public AxisService findService(MessageContext messageContext)
    	throws AxisFault
    {
    	return serviceDispatcher.findService(messageContext);
    }
	...
	public InvocationResponse invoke(MessageContext msgctx) throws AxisFault {
        ...
        if (msgctx.getRelatesTo() != null) {
            ...
            if (LoggingControl.debugLoggingAllowed && log.isDebugEnabled()) {
                log.debug(msgctx.getLogCorrelationID() + " " + Messages.getMessage("checkingrelatesto",
                                                                              relatesTo));
            }
...
//                    }
                    ...
                    msgctx.setServiceContext(operationContext.getParent());
                    msgctx.setAxisService(
                            (operationContext.getParent()).getAxisService());

                    // TODO : Is this necessary here?
                    msgctx.getAxisOperation().registerMessageContext(msgctx, operationContext);

                    msgctx.setServiceGroupContextId(
                            (msgctx.getServiceContext().getParent()).getId());
        ...
        //Else we will try to dispatch based on the WS-A Action
            ...
            if (JavaUtils.isTrueExplicitly(flag)) {
            	...
            	//phase and addressing is in use we should throw an
            	//ActionNotSupported Fault, unless we've been told not to do
            	//this check (by Synapse, for instance)
    ...
    /**
     * If addressing was found and the dispatch failed we SHOULD return a
     * WS-Addressing ActionNotSupported fault. This will make more sense once
     * the AddressingBasedDsipatcher is moved into the addressing module
     */
