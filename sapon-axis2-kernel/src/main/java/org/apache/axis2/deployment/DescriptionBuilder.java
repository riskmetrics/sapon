...
public class DescriptionBuilder implements DeploymentConstants {
    ...
    private static final QName ATTRIBUTE_QNAME = new QName(ATTRIBUTE_NAME);
     ...
     *
     */
    protected Map<String, MessageReceiver> processMessageReceivers(OMElement messageReceivers)
            throws DeploymentException {
        HashMap<String, MessageReceiver> mr_mep = new HashMap<String, MessageReceiver>();
        for(OMElement msgReceiver: messageReceivers.getChildrenWithName(new QName(TAG_MESSAGE_RECEIVER))) {
     ...
     *
     */
    protected Map<String, MessageReceiver> processMessageReceivers(ClassLoader loader,
                                              OMElement element) throws DeploymentException {
        Map<String, MessageReceiver> meps = new HashMap<String, MessageReceiver>();
        for(OMElement receiverElement: element.getChildrenWithName(new QName(TAG_MESSAGE_RECEIVER))) {
    ...
    protected Map<String, Builder> processMessageBuilders(OMElement messageBuildersElement)
            throws DeploymentException {
        Map<String, Builder> builderSelector = new HashMap<String, Builder>();
        for(OMElement msgBuilderElement: messageBuildersElement.getChildrenWithName(new QName(TAG_MESSAGE_BUILDER))) {
    ...
    protected Map<String, MessageFormatter> processMessageFormatters(OMElement messageFormattersElement)
            throws DeploymentException {
        Map<String, MessageFormatter> messageFormatters
        	= new HashMap<String, MessageFormatter>();

        for(OMElement msgFormatterElement: messageFormattersElement.getChildrenWithName(new QName(TAG_MESSAGE_FORMATTER))) {
    ...
    protected Flow processFlow(OMElement flowelement, ParameterInclude parent)
        ...
        for(OMElement handlerElement: flowelement.getChildrenWithName(new QName(TAG_HANDLER))) {
    ...
    protected QName[] getLocalPolicyAssertionNames(OMElement localPolicyAssertionsElement) {
        ...
        Iterator<OMElement> iterator = localPolicyAssertionsElement.getChildElements().iterator();
        if (! iterator.hasNext()) {
            return null;
        }
    ...
    protected HandlerDescription processHandler(OMElement handler_element,
        ...
        if (order_element == null) {
            ...
            for(OMAttribute orderAttribute: order_element.getAllAttributes()) {
            ...
            Iterable<OMElement> parameters = handler_element
    ...
    protected void processOperationModuleRefs(Iterable<OMElement> moduleRefs,
     ...
     */
    protected void processParameters(Iterable<OMElement> parameters,
                                     ParameterInclude parameterInclude,
                                     ParameterInclude parent)
            throws DeploymentException {
    	// this is to check whether some one has locked the parmeter at the
    	// top level
        for(OMElement parameterElement: parameters) {
            Parameter parameter = new Parameter();
            // setting parameterElement
            parameter.setParameterElement(parameterElement);
            // setting parameter Name
            OMAttribute paramName
            	= parameterElement.getAttribute(ATTRIBUTE_QNAME);
            if (paramName == null) {
                throw new DeploymentException(Messages.getMessage(
    ...
    protected void processPolicyElements(Iterable<OMElement> policyElements,
                                         PolicySubject policySubject) {
        for(OMElement elem: policyElements) {
            Policy p = PolicyEngine.getPolicy(elem);
            policySubject.attachPolicy(p);
        }
    }

    protected void processPolicyRefElements(Iterable<OMElement> policyRefElements,
                                            PolicySubject policySubject) {
        for(OMElement elem: policyRefElements) {
            PolicyReference policyReference
            	= PolicyEngine.getPolicyReference(elem);
