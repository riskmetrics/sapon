...
public class Wss11 extends Wss10 {
	...
	public void serialize(XMLStreamWriter writer) throws XMLStreamException {
        ...
        if (isMustSupportRefKeyIdentifier()) {
            writer.writeStartElement(namespaceURI, SPConstants.MUST_SUPPORT_REF_KEY_IDENTIFIER);
            writer.writeEndElement();
        }

        if (isMustSupportRefIssuerSerial()) {
            // <sp:MustSupportRefIssuerSerial />
            writer.writeStartElement(namespaceURI, SPConstants.MUST_SUPPORT_REF_ISSUER_SERIAL);
            writer.writeEndElement();
        }

        if (isMustSupportRefExternalURI()) {
            // <sp:MustSupportRefExternalURI />
            writer.writeStartElement(namespaceURI, SPConstants.MUST_SUPPORT_REF_EXTERNAL_URI);
            writer.writeEndElement();
        }

        if (isMustSupportRefEmbeddedToken()) {
            // <sp:MustSupportRefEmbeddedToken />
            writer.writeStartElement(namespaceURI, SPConstants.MUST_SUPPORT_REF_EMBEDDED_TOKEN);
            writer.writeEndElement();
        }

        if (isMustSupportRefThumbprint()) {
            // <sp:MustSupportRefThumbprint />
            writer.writeStartElement(namespaceURI, SPConstants.MUST_SUPPORT_REF_THUMBPRINT);
            writer.writeEndElement();
        }

        if (isMustSupportRefEncryptedKey()) {
            // <sp:MustSupportRefEncryptedKey />
            writer.writeStartElement(namespaceURI, SPConstants.MUST_SUPPORT_REF_ENCRYPTED_KEY);
            writer.writeEndElement();
        }

        if (isRequireSignatureConfirmation()) {
            // <sp:RequireSignatureConfirmation />
            writer.writeStartElement(namespaceURI, SPConstants.REQUIRE_SIGNATURE_CONFIRMATION);
            writer.writeEndElement();
        }
