...
public abstract class SOAPHeaderImpl extends SOAPElement implements SOAPHeader {
     ...
     */
    public Iterable<SOAPHeaderBlock> getHeadersToProcess(RolePlayer rolePlayer) {
        return null; // TODO: Implement this!
    }

    public Iterable<SOAPHeaderBlock> getHeadersToProcess(RolePlayer rolePlayer, String namespace) {
        return null; // TODO: Implement this!
    }
     ...
     */
    public Iterable<SOAPHeaderBlock> examineHeaderBlocks(String paramRole) {
       ...
       return headersWithGivenActor.iterator();*/
        ...
        Collection<SOAPHeaderBlock> elements = new ArrayList<SOAPHeaderBlock>();
        for(SOAPHeaderBlock headerBlock: examineAllHeaderBlocks()) {
            /*
...
//    }
     ...
     */
    public abstract Iterable<SOAPHeaderBlock> extractHeaderBlocks(String role);
     ...
     */
    public Iterable<SOAPHeaderBlock> examineMustUnderstandHeaderBlocks(String actor) {
        List<SOAPHeaderBlock> mustUnderstandHeadersWithGivenActor
        	= new ArrayList<SOAPHeaderBlock>();
        for(OMNode node: getChildren()) {
            if (node instanceof SOAPHeaderBlock) {
                SOAPHeaderBlock soapHeaderBlock = (SOAPHeaderBlock)node;
                String role = soapHeaderBlock.getRole();
        ...
        return mustUnderstandHeadersWithGivenActor;
    }

    /**
     * Returns an <code>Iterable</code> over all the <code>SOAPHeaderBlock</code> objects in this
     ...
     */
    public Iterable<SOAPHeaderBlock> examineAllHeaderBlocks() {
        return new OMFilterIterator<SOAPHeaderBlock>(this.getChildrenWithName(null).iterator()) {
			@Override
			protected SOAPHeaderBlock matches(OMNode node) {
				return (SOAPHeaderBlock)node;
			}
        };
    }
     ...
     *         contained by this <code>SOAPHeader</code>
     */
    public Iterable<SOAPHeaderBlock> extractAllHeaderBlocks() {
        Collection<SOAPHeaderBlock> result = new ArrayList<SOAPHeaderBlock>();
        for (OMElement elem: getChildrenWithName(null)) {
            result.add((SOAPHeaderBlock)elem.detach());
        }
        return result;
    }

    public List<SOAPHeaderBlock> getHeaderBlocksWithNSURI(String nsURI) {
        List<SOAPHeaderBlock> headers = null;
        OMNode node;
        OMElement header = this.getFirstElement();

        if (header != null) {
            headers = new ArrayList<SOAPHeaderBlock>();
        }
        ...
        while (node != null) {
            if (node.getType() == OMNode.ELEMENT_NODE) {
                ...
                if (nsURI == null) {
                    if (namespace == null) {
                        headers.add((SOAPHeaderBlock)header);
                    }
                } else {
                    if (namespace != null) {
