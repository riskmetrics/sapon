...
public class ServiceGroupContext extends AbstractContext<ConfigurationContext>
    implements Externalizable, SafeSerializable
{

    private static final Log log
    	= LogFactory.getLog(ServiceGroupContext.class);
    ...
    public ServiceContext findServiceContext(AxisService axisSrv) {
        ...
        return serviceContextMap.get(axisSrv.getName());
    }
    ...
    */
    ...
    public void readExternal(ObjectInput inObject) throws IOException, ClassNotFoundException {
        ...
        if (log.isTraceEnabled()) {
            log.trace(this.getClass().getName() + ":readExternal():  BEGIN " +
            		  "bytes available in stream [" + in.available() + "]  ");
        }
    ...
    private void checkActivateWarning(String methodname) {
        if (needsToBeReconciled) {
            log.warn(getClass().getName() + ":" + methodname + "(): ****WARNING**** " + getClass().getName() +
