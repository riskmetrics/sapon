...
/**
 * <p>Axis2 states are held in two information models, called description
 * hierarchy and context hierarchy. Description hierarchy hold deployment
 * configuration and it's values does not change unless deployment
 * configuration change occurs where Context hierarchy hold run time
 * information.  Both hierarchies consists four levels, Global, Service Group,
 * Operation and Message. Please look at "Information Model" section of
 * "Axis2 Architecture Guide" for more information.</p>
 * <p/>
 ...
 * configurations can changed by setting name value pairs of the
 * configurationContext. This hold all OperationContexts, ServiceGroups,
 * Sessions, and ListenerManager.
 */
public class ConfigurationContext extends AbstractContext<ConfigurationContext> {

    private static final Log log
    	= LogFactory.getLog(ConfigurationContext.class);
    ...
    private final ConcurrentHashMap<String, OperationContext> operationContextMap
    	= new ConcurrentHashMap<String, OperationContext>();
...
//                                            " for the serviceGroupId: " + serviceGroupContextId);
    ...
    public boolean registerOperationContext(String messageID,
        ...
        if (log.isDebugEnabled()) {
            ...
            Map<String, MessageContext> msgContextMap = mepContext.getMessageContexts();
     ...
     *
     * @return a new OldMessageContext
     */
    public MessageContext createMessageContext() {
        MessageContext msgCtx = new OldMessageContext(this);
    ...
    private void cleanupServiceContexts(ServiceGroupContext serviceGroupContext) {
        ...
        Iterable<ServiceContext> serviceContexts = serviceGroupContext.getServiceContexts();
        if (serviceContexts == null) {
            return;
        }
        for(final ServiceContext serviceContext: serviceContexts) {
