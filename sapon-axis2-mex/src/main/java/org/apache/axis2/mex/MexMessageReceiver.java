...
public class MexMessageReceiver extends AbstractInOutMessageReceiver {
	...
	private Metadata handleRequest(MessageContext msgContext) throws AxisFault {
		...
		List<String> metadata_request_list;
		if (aReq != null) {
	...
	public Metadata processRequest(List<String> metadata_request_list,
		...
		for (int i = 0; i < len; i++) { // metadata request
            ...
            String dialect = "";
			try {
				dialect = metadata_request_list.get(i);
				...
				for (OutputForm outputform : outputforms) { // output form
					requestOptions.putOutputForm(outputform);

					Data[] result =  msgContext.getAxisService().getData(requestOptions,
							msgContext);

					List<MetadataSection> sections
						= processData(result, outputform, dialect, identifier_value, factory);
					metadata.addMetadataSections(sections);
				}
	 ...
	 */
	private List<MetadataSection> processData(Data[] data, OutputForm outputForm,
			String dialect, String identifier_value, SOAPFactory factory) throws MexException {
		MetadataSection section=null;
		List<MetadataSection> sections = new ArrayList<MetadataSection>();
		if (data == null || data.length == 0) {
			...
			for (Data element : data) {
				...
				section = createMetadataSection(outputForm, element.getData(),
						factory, mexNamespaceValue);

				section.setDialect(dialect);
				identifier_value = element.getIdentifier();
	 ...
	 *
	 */
	private List<String> determineMetadataTypes(OMElement aReq) {
		List<String> metadata_request_list = new ArrayList<String>();
