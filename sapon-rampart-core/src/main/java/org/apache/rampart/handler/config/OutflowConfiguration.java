...
public class OutflowConfiguration {
	...
	public Parameter getProperty() {
        ...
        for (HashMap element : actionList) {
            HashMap action = element;
		...
		for (HashMap action : this.actionList) {
		...
		Parameter param = new Parameter(WSSHandlerConstants.OUTFLOW_SECURITY,
										paramElement);
		return param;
	}
