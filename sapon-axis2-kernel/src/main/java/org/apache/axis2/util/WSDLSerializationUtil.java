...
public class WSDLSerializationUtil {
     ...
     */
    public static String getElementName(AxisMessage axisMessage, Map<String, String> nameSpaceMap) {
     ...
     * @param nameSpaceMap - The namespaceMap
     */
    public static void addSOAPHeaderElements(OMFactory omFactory, List<SOAPHeaderMessage> list, OMNamespace wsoap,
                                             OMElement element, Map<String, String> nameSpaceMap) {
        for (int i = 0; i < list.size(); i++) {
            SOAPHeaderMessage soapHeaderMessage = list.get(i);
     ...
     * @param element - The element that the header should be added to
     */
    public static void addSOAPModuleElements(	final OMFactory omFactory,
    											final List<SOAPModuleMessage> list,
    											final OMNamespace wsoap,
    											final OMElement element )
    {
        for(SOAPModuleMessage soapModuleMessage: list) {
            final OMElement soapModuleElement =
     ...
     * @param nameSpaceMap - The namespaceMap
     */
    public static void addHTTPHeaderElements(OMFactory omFactory, List<HTTPHeaderMessage> list, OMNamespace whttp,
                                             OMElement element, Map<String, String> nameSpaceMap) {
        for (final HTTPHeaderMessage httpHeaderMessage: list) {
    ...
    public static OMElement generateHTTPBinding(OMFactory fac, AxisService axisService,
        ...
        for(final AxisDescription desc: axisService.getChildren()) {
            ...
            final String name = axisOperation.getName().getLocalPart();
            opElement.addAttribute(fac.createOMAttribute(
            		WSDL2Constants.ATTRIBUTE_REF, null,
            		tns.getPrefix() + ":" + name));
            opElement.addAttribute(fac.createOMAttribute(
            		WSDL2Constants.ATTRIBUTE_LOCATION, whttp, name));
        }
        return binding;
    }
     ...
     * @param nameSpaceMap - The namespaceMap
     */
    public static void populateNamespaces(OMElement descriptionElement, Map<String, String> nameSpaceMap) {
        ...
        for(Map.Entry<String, String> e: nameSpaceMap.entrySet()) {
            if ("".equals(e.getKey())) {
                descriptionElement.declareDefaultNamespace(e.getValue());
            } else {
                descriptionElement.declareNamespace(e.getValue(), e.getKey());
            }
        }
    }
    ...
    public static void addPoliciesAsExtensibleElement(
		...
		for (Object policyElement : attachPolicyComponents) {
			if (policyElement instanceof Policy) {
				policies.add((Policy)policyElement);
