...
public class ServerManager {
    ...
    /**
     * The controller for synapse create and Destroy synapse artifacts in a
     * particular environment
     * Only for internal usage - DON'T PUT GETTER OR SETTER
     */
     ...
     * When JMX calls are received via RMI connections, re-start etc may
     * otherwise fail due to classloading issues.
     */
     ...
     * calling this will create the new axis2 environment, but this won't start
     ...
     */
    public synchronized ServerState init(	SynapseServerInfo synapseServerInfo,
            								ServerContextInformation serverContextInformation )
    {
        ...
        synapseController = SynapseControllerFactory
                .createSynapseController(synapseServerInfo);
    ...
    public synchronized ServerState shutdown() {
        ...
        ServerState serverState = ServerStateDetectionStrategy.currentState(
                serverContextInformation, synapseServerInfo);
     ...
     */
    public SynapseServerInfo getServerConfigurationInformation() {
        assertInitialized();
        return synapseServerInfo;
    }
    ...
    private void doInit() {
        ...
        // if the server is ready for the initialization, this will make sure
        // that we are not calling the initialization on an already
        // initialized/started system
        if (serverState == ServerState.INITIALIZABLE) {
            ...
            this.synapseController.init(synapseServerInfo, serverContextInformation);
