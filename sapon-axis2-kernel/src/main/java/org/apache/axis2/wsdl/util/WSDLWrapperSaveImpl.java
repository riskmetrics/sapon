...
public class WSDLWrapperSaveImpl implements WSDLWrapperImpl {
    ...
    private boolean save() {
        ...
        if (savedDefinitionFile == null) {
            try {
                ...
                log.debug(getClass().getName() + ".save(): temp file = [" + savedFilename + "]");
            }
            catch (Exception ex) {
                log.debug(getClass().getName() + ".save(): error creating temp file = [" + ex.getMessage() + "]");
        ...
        if (savedDefinitionFile != null) {
            ...
            try {
                ...
                log.debug(getClass().getName() + ".save(): saving the wsdl definition.....");
                ...
                log.debug(getClass().getName() + ".save(): ....saved the wsdl definition.....");

                long filesize = savedDefinitionFile.length();
                log.debug(getClass().getName() + ".save(): file size after save [" + filesize +
                        "]   temp file = [" + savedFilename + "]");

                log.debug(getClass().getName() + ".save(): end - - - - - - - - - - - - - - - -");
                return true;
                ...
                log.debug(getClass().getName() + ".save(): error with saving the wsdl definition = [" +
                ...
                log.debug(getClass().getName() + ".save(): error exit - - - - - - - - - - - - - - - -");
                return false;
            }
        }
    ...
    private Definition restore() {
        ...
        try {
            ...
            log.debug(getClass().getName() + ".restore(): restoring the WSDL definition .....");

            restoredDefinition = (Definition) inObjStream.readObject();
            inObjStream.close();
            inStream.close();

            log.debug(getClass().getName() + ".restore(): ....restored the WSDL definition .....");

        }
        catch (Exception ex2) {
            log.debug(getClass().getName() + ".restore(): error with restoring the WSDL definition = [" +
                    ex2.getClass().getName() + " : " + ex2.getMessage() + "]", ex2);
        ...
        log.debug(getClass().getName() + ".restore(): end - - - - - - - - - - - - - - - -");
