...
public class RampartUtil {
   ...
   public static RampartConfigCallbackHandler getRampartConfigCallbackHandler(MessageContext msgContext,
    ...
    private static Set findAllPrefixNamespaces(OMElement currentElement, Map<String, String> decNamespacess)
    {
    	...
    	for (Object element : decNamespacess.keySet()) {
    ...
    private static void findPrefixNamespaces(OMElement e, Set<OMNamespace> results)
    {
    	...
    	for(OMElement elem: e.getChildElements()) {
    		findPrefixNamespaces(elem, results);
	    }
    }
    ...
    public static Vector getContentEncryptedElements (Vector encryptedPartsElements, SOAPEnvelope envelope,Vector elements, Map<String, String> decNamespaces ) {
    ...
    public static Vector getPartsAndElements(boolean sign, SOAPEnvelope envelope, boolean includeBody, Vector parts, Vector elements, Map<String, String> decNamespaces) {
        ...
        for(int i=0; i<parts.size(); i++) {
            ...
            if( wsep.getName() == null ) {
                ...
                for(SOAPHeaderBlock shb: header.getHeaderBlocksWithNSURI( wsep.getNamespace() )) {
    ...
    public static String getSigElementId(RampartMessageData rmd) {
        ...
        List<SOAPHeaderBlock> secHeaders = header.getHeaderBlocksWithNSURI(WSConstants.WSSE_NS);
        ...
        if (secHeaders != null && secHeaders.size() > 0) {
            ...
            OMElement sigElem = (secHeaders.get(0)).getFirstChildWithName(sigQName);
