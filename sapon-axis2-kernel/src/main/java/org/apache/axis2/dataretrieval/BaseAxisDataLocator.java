...
public abstract class BaseAxisDataLocator {
    	...
    	throws DataRetrievalException
    {
        ...
        for(ServiceData sd: serviceData) {
        	final OMElement metaElement = sd.getFileContent(classLoader);
        	if (metaElement != null) {
    	...
    	throws DataRetrievalException
    {
        ...
        final List<Data> result = new ArrayList<Data>();
        for (ServiceData element: serviceData) {
        	final String urlValue = element.getURL();
        	if (urlValue != null) {
        		result.add(new Data(urlValue, element.getIdentifier()));
        	}
        }
        return result.toArray(new Data[result.size()]);
    }
    ...
    protected Data[] outputReferenceForm(MessageContext msgContext,
        ...
        for (ServiceData element : serviceData) {
        	final OMElement epr = element.getEndpointReference();
        	if (epr != null) {
        		result.add(new Data(epr, element.getIdentifier()));
            }
        }
        return result.toArray(new Data[result.size()]);
    }
