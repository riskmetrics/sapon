...
public class DocumentNavigator extends DefaultNavigator {
    ...
    private StringBuffer getStringValue(OMNode node, StringBuffer buffer) {
        if (isText(node)) {
            ...
            for(OMNode child: ((OMElement) node).getChildren()) {
     ...
     */
    @Override
	public Iterator<OMNode> getChildAxisIterator(Object contextNode) throws UnsupportedAxisException {
        if (contextNode instanceof OMContainer) {
            return ((OMContainer) contextNode).getChildren().iterator();
        }
        return new EmptyIterator<OMNode>();
    }
     ...
     */
    @Override
	public Iterator<OMAttribute> getAttributeAxisIterator(Object contextNode) throws UnsupportedAxisException {
        if (isElement(contextNode)) {
            ArrayList<OMAttribute> attributes = new ArrayList<OMAttribute>();
            for(OMAttribute attr: ((OMElement) contextNode).getAllAttributes()) {
                attributes.add(new OMAttributeEx(attr,
     ...
     */
    @Override
	public Iterator<OMNamespace> getNamespaceAxisIterator(Object contextNode) throws UnsupportedAxisException {
        if (!(contextNode instanceof OMContainer &&
                contextNode instanceof OMElement)) {
            return new EmptyIterator<OMNamespace>();
        }
        List<OMNamespace> nsList = new ArrayList<OMNamespace>();
        ...
        for (OMContainer context = (OMContainer) contextNode;
            ...
            ArrayList<OMNamespace> declaredNS = new ArrayList<OMNamespace>();
     ...
     */
    @Override
	public Iterator<OMNode> getFollowingSiblingAxisIterator(Object contextNode)
            throws UnsupportedAxisException {
     ...
     */
    @Override
	public Iterator<OMNode> getPrecedingSiblingAxisIterator(Object contextNode)
            throws UnsupportedAxisException {
    ...
    static class OMAttributeEx implements OMAttribute {
        ...
        public void setNamespace(OMNamespace omNamespace) {
            attribute.setNamespace(omNamespace);
        }
