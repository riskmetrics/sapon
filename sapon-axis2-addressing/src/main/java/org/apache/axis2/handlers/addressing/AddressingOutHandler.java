...
public class AddressingOutHandler extends AbstractHandler implements AddressingConstants {
    ...
    public InvocationResponse invoke(MessageContext msgContext) throws AxisFault {
        ...
        if (disableAddressing) {
            if (LoggingControl.debugLoggingAllowed && log.isTraceEnabled()) {
                log.trace(msgContext.getLogCorrelationID() +
    ...
    private class WSAHeaderWriter {
        ...
        private final Options messageContextOptions;
        private OMNamespace addressingNamespaceObject;
        private final String addressingNamespace;
        private String addressingRole;

        private final boolean isFinalAddressingNamespace;
        private final boolean addMustUnderstandAttribute;
        private final boolean replaceHeaders;  // determines whether we replace the existing headers or not, if they present
        private final boolean includeOptionalHeaders;

        private List<String> existingWSAHeaders = null;
        ...
        public WSAHeaderWriter(MessageContext mc, boolean isSubmissionNamespace, boolean addMU,
            ...
            if (header == null) {
            	...
            	if(addressingHeaders!=null && !addressingHeaders.isEmpty()){
            		existingWSAHeaders = new ArrayList<String>(addressingHeaders.size());
        ...
        private void processMessageID() {
            ...
            {//optional
            	List<OMAttribute> attributes = (List<OMAttribute>)messageContext.getLocalProperty(
        ...
        private void processWSAAction() throws AxisFault {
            ...
            if (LoggingControl.debugLoggingAllowed && log.isTraceEnabled()) {
                log.trace(messageContext.getLogCorrelationID() +
        ...
        private void processRelatesTo() {
            if (!isAddressingHeaderAlreadyAvailable(WSA_RELATES_TO, true)) {
                ...
                if (relatesTo != null) {
                    for (RelatesTo element : relatesTo) {
                        OMElement relatesToHeader = createSOAPHeaderBlock(element.getValue(),
                        		WSA_RELATES_TO, element.getExtensibilityAttributes());
                        String relationshipType = element.getRelationshipType();
        ...
        private OMElement createSOAPHeaderBlock(String value, String headerName, List<OMAttribute> attributes) {
            if (LoggingControl.debugLoggingAllowed && log.isTraceEnabled()) {
            ...
            if (value != null && value.length()!=0) {
                ...
                if (attributes != null) {
                    for(OMAttribute attr: attributes) {
                        AttributeHelper.importOMAttribute(attr, soapHeaderBlock);
                    }
                }
         ...
         *                             attached
         */
        private void processToEPRReferenceInformation(Map<?, OMElement> referenceInformation) throws Exception {
            ...
            if(service != null){
            	...
            	if(endpoint != null){
            		List<OMElement> referenceparameters = (List<OMElement>) endpoint.getParameterValue(REFERENCE_PARAMETER_PARAMETER);
            		if (LoggingControl.debugLoggingAllowed && log.isTraceEnabled()) {
        ...
        private boolean isAddressingHeaderAlreadyAvailable(String name, boolean multipleHeaders) {
        	...
        	if (multipleHeaders) {
        		if (replaceHeaders) {
        			...
        			for(OMElement addressingHeader: header.getChildrenWithName(qname)) {
        ...
        private void processMustUnderstandProperty() {
            if (addMustUnderstandAttribute) {
                List<SOAPHeaderBlock> headers = header.getHeaderBlocksWithNSURI(addressingNamespace);
