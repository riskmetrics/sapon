...
//                        !msgContext.isPropertyTrue(Constants.Configuration.SOAP_RESPONSE_MEP)) {
            ...
            if (builder != null) {
                ...
                documentElement = builder.getDocumentElement(inStream, contentType, msgContext);
            }
        }
    ...
    private static String getMessageFormatterProperty(MessageContext msgContext) {
       ...
       public static void deleteAttachments(MessageContext msgContext) {
       	...
       	Attachments attachments = msgContext.getAttachments();
    ...
    public static boolean doWriteMTOM(MessageContext msgContext) {
        ...
        if (!enableMTOM && msgContext.isDoingMTOM() && (enableMTOMObject instanceof String)) {
            if (((String) enableMTOMObject).equalsIgnoreCase(Axis2Constants.VALUE_OPTIONAL)) {
            	...
            	//This was the old code, dependent on isServerSide().  Note
            	//that since isDoingMTOM() was already checked above,
            	//enableMTOM was set to true regardless of what isServerSide()
            	//evaluated to.
                /*if (msgContext.isServerSide()) {
            	...
            	//So, because of that, just set enableMTOM to true
            	//and be done with it, and have one less place dependent on
            	//isServerSide().
            	enableMTOM = true;
            }
        }
