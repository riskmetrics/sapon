...
public class TokenRequestDispatcherConfig {
    ...
    private Map<String, String> renewers;
    private Map<String, String> configFiles = new Hashtable<String, String>();
    private Map<String, OMElement> configElements = new Hashtable<String, OMElement>();
    private Map<String, String> configParamNames = new Hashtable<String, String>();
    ...
    private static void handleIssuers(OMElement configElem,
                                      TokenRequestDispatcherConfig conf) throws TrustException {
        for(OMElement element: configElem.getChildrenWithName(ISSUER)) {
            ...
            //Process token types
            for(OMElement type: element.getChildrenWithName(TOKEN_TYPE)) {
                ...
                if (conf.issuers == null) {
                    conf.issuers = new Hashtable<String, String>();
                }
    ...
    private static void handleValidators(OMElement configElem,
        ...
        for(OMElement element: configElem.getChildrenWithName(VALIDATOR)) {
           ...
           for(OMElement type: element.getChildrenWithName(TOKEN_TYPE)) {
               ...
               if (conf.validators == null) {
                   conf.validators = new Hashtable<String, String>();
               }
               //If the token type is not already declared then add it to the
    ...
    private static void handleRenewers(OMElement configElem,
        ...
        for(OMElement element: configElem.getChildrenWithName(RENEWER)) {
           ...
           for(OMElement type: element.getChildrenWithName(TOKEN_TYPE)) {
               ...
               if (conf.renewers == null) {
                   conf.renewers = new Hashtable<String, String>();
               }
               //If the token type is not already declared then add it to the
    ...
    private static void processConfiguration(OMElement element,
                                             ...
                                             String implClass) {
        for(OMElement configEle: element.getChildrenWithName(CONFIGURATION_ELEMENT)) {
            String configType =
    ...
    public TokenIssuer getIssuer(String tokenType) throws TrustException {
        ...
        if (this.issuers != null) {
            issuerClassName = this.issuers.get(tokenType);
        }
    ...
    public TokenValidator getValidator(String tokenType) throws TrustException {
        ...
        if (this.validators != null) {
            validatorClassName = this.validators.get(tokenType);
        }
    ...
    public TokenRenewer getRenewer(String tokenType) throws TrustException {
        ...
        if (this.renewers != null) {
            renewerClassName = this.renewers.get(tokenType);
        }
    ...
    private TokenIssuer createIssuer(String issuerClassName) throws Exception {
        ...
        issuer.setConfigurationElement(this.configElements.get(issuerClassName));
        issuer.setConfigurationFile(this.configFiles.get(issuerClassName));
        issuer.setConfigurationParamName(this.configParamNames.get(issuerClassName));
        return issuer;
    }
    ...
    private TokenCanceler createCanceler(String cancelerClassName) throws Exception {
        ...
        canceler.setConfigurationElement(this.configElements.get(cancelerClassName));
        canceler.setConfigurationFile(this.configFiles.get(cancelerClassName));
        canceler.setConfigurationParamName(this.configParamNames.get(cancelerClassName));
        return canceler;
    }
    ...
    private TokenValidator createValidator(String validatorClassName) throws Exception {
        ...
        validator.setConfigurationElement(this.configElements.get(validatorClassName));
        validator.setConfigurationFile(this.configFiles.get(validatorClassName));
        validator.setConfigurationParamName(this.configParamNames.get(validatorClassName));
        return validator;
    }
    ...
    private TokenRenewer createRenewer (String renewerClassName) throws Exception {
        ...
        renewer.setConfigurationElement(this.configElements.get(renewerClassName));
        renewer.setConfigurationFile(this.configFiles.get(renewerClassName));
        renewer.setConfigurationParamName(this.configParamNames.get(renewerClassName));
        return renewer;
    }
}
