...
public class ValidateMediator extends AbstractListMediator {
	...
	public boolean mediate(SynapseMessageContext synCtx) {
        ...
        log.debug("Start : Validate mediator");
        if (log.isTraceEnabled()) {
            log.trace("Message : " + synCtx.getEnvelope());
        }

        // Input source for the validation
        Source validateSrc = getValidationSource(synCtx);
            ...
            if (reCreate || cachedSchema == null) {
                ...
                if (errorHandler.isValidationError()) {
                    ...
                    if (log.isDebugEnabled()) {
                        log.debug("Error creating a new schema objects for " +
        ...
        try {
            ...
            if (errorHandler.isValidationError()) {
                ...
                if (log.isDebugEnabled()) {
                    ...
                    if (log.isTraceEnabled()) {
                        log.trace("Failed message envelope : " + synCtx.getEnvelope());
                    }
                }
        ...
        log.debug("End : Validate mediator");
     ...
     */
    private Source getValidationSource(SynapseMessageContext synCtx) {

        try {
            OMNode validateSource = source.selectOMNode(synCtx);
            if (log.isDebugEnabled()) {
                log.debug("Validation source : " + validateSource);
            }
