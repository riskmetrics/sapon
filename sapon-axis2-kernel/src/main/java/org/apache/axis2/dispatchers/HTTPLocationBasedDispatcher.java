...
public class HTTPLocationBasedDispatcher extends AbstractDispatcher {
            ...
            if (httpLocation != null) {
                ...
                if (axisEndpoint != null) {
                    Map<String, AxisOperation> httpLocationTable
                    	= (Map<String, AxisOperation>) axisEndpoint.getBinding()
     ...
     *
     * @param requestPath - Part of the request url which is the part after the
     *                      service name
     * @param httpLocationTable - The httpLocationTable stored in the relevant
     *                            binding
     * @return AxisOperation - The corresponding AxisOperation
     */
    private AxisOperation getOperationFromHTTPLocation(	String requestPath,
    													Map<String, AxisOperation> httpLocationTable) {
    	for(Map.Entry<String, AxisOperation> e: httpLocationTable.entrySet()) {
            int index = requestPath.indexOf(e.getKey());
            if (index == 0) {
                return e.getValue();
            }
        }
