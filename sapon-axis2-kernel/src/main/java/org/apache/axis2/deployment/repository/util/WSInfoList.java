...
public class WSInfoList implements DeploymentConstants {
     ...
     */
    public Map<String, WSInfo> currentJars = new HashMap<String, WSInfo>();
    ...
    public synchronized void addWSInfoItem(File file, Deployer deployer , int type) {
        WSInfo info = currentJars.get(file.getAbsolutePath());
        if (info != null) {
    ...
    private synchronized void checkForUndeployedServices() {
        ...
        Iterator<String> infoItems = currentJars.keySet().iterator();
        List<String> tobeRemoved = new ArrayList<String>();
        while (infoItems.hasNext()) {
            String  fileName = infoItems.next();
            WSInfo infoItem = currentJars.get(fileName);
        ...
        for (int i = 0; i < tobeRemoved.size(); i++) {
            String fileName = tobeRemoved.get(i);
    ...
    private WSInfo getFileItem(File file , Deployer deployer , int type) {
        ...
        WSInfo info = currentJars.get(fileName);
        if(info==null){
