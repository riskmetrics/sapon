...
public class ConfigurationContextFactory {
    ...
    private static void addModuleService(ConfigurationContext configCtx) throws AxisFault {
        ...
        Map<String, AxisModule> modules = axisConfig.getModules();
        if (modules != null) {
            for(final AxisModule axisModule: modules.values()) {
    ...
    private static void initModules(ConfigurationContext context) {
        ...
        final Map<String, AxisModule> modules = configuration.getModules();
        final Collection<AxisModule> col = modules.values();
        final Map<AxisModule, AxisFault> faultyModule
        	= new HashMap<AxisModule, AxisFault>();

        for (AxisModule axismodule : col) {
            final Module module = axismodule.getModule();
            if (module != null) {
                try {
        ...
        //Checking whether we have found any faulty services during the module
        //initilization, if so we need to mark them as fautyModule and need to
        //remove from the modules list
        for(final Map.Entry<AxisModule, AxisFault> e: faultyModule.entrySet()) {
        	final AxisModule axisModule = e.getKey();
        	String fileName;
        	...
        	configuration.getFaultyModules().put(fileName, e.getValue().toString());
    ...
    private static void initTransportSenders(ConfigurationContext configContext) {
        ...
        final Map<String, TransportOutDescription> transportOuts
        	= axisConf.getTransportsOut();

        for(final TransportOutDescription transportOut: transportOuts.values()) {
